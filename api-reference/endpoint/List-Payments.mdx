...
If the same call produced multiple failed attempts, only the most recent failed attempt is returned. Successful payments are never collapsed — a failed attempt followed by a success on the same call returns both rows, and a partial payment plus its remainder charge returns both rows.
